...
classDiagram
    ...
    class ITradingClient {
        ...
        +list_positions(Optional~Any~ account) Dict~str, Any~*
    ...
    class ShioajiClient {
        ...
        +cancel_order(str order_id) Dict~str, Any~
        +update_order(str order_id, float price, int quantity) Dict~str, Any~
        +get_deal_report(Optional~str~ order_id) Dict~str, Any~
        +get_latest_deal_report() Dict~str, Any~
        +get_order_report(Optional~str~ order_id) Dict~str, Any~
        +get_latest_order_report() Dict~str, Any~
        +get_orders_by_status(str status) Dict~str, Any~
        +get_account_balance() Dict~str, Any~
        +list_positions(Optional~Any~ account) Dict~str, Any~
    }
    ...
    class OrderDealCallbackHandler {
        ...
        +get_deals() List~Dict~
        +get_latest_deal() Optional~Dict~
        +get_deals_by_order_id(str order_id) List~Dict~
        +get_total_deal_quantity(Optional~str~ order_id) int
        +get_average_deal_price(Optional~str~ order_id) float
        +clear_deals(Optional~str~ order_id) None
        +clear_orders(Optional~str~ order_id) None
        +get_latest_order() Optional~Dict~
        +get_orders_by_status(str status) List~Dict~
        +get_order_by_id(str order_id) Optional~Dict~
        +get_order_statistics() Dict~str, Any~
    }
...
- `get_account_balance()`: 取得帳戶餘額（抽象方法）
- `list_positions()`: 取得持倉資訊（抽象方法）
...
- `cancel_order()`: 取消訂單
- `update_order()`: 修改訂單
- `get_account_balance()`: 取得帳戶餘額
- `list_positions()`: 取得持倉資訊
- `get_deal_report()`: 取得成交回報
- `get_latest_deal_report()`: 取得最新成交回報
- `get_order_report()`: 取得委託回報
- `get_latest_order_report()`: 取得最新委託回報
- `get_orders_by_status()`: 根據狀態取得委託回報
- `get_account_balance()`: 取得帳戶餘額
- `list_positions()`: 取得持倉資訊
...
- `get_orders()`: 取得所有委託記錄
- `get_deals()`: 取得所有成交記錄
- `get_latest_deal()`: 取得最新成交記錄
- `get_deals_by_order_id()`: 根據訂單 ID 取得成交記錄
- `get_total_deal_quantity()`: 取得成交總數量
- `get_average_deal_price()`: 取得平均成交價格
- `clear_deals()`: 清除成交記錄
- `clear_orders()`: 清除委託記錄
- `get_latest_order()`: 取得最新委託記錄
- `get_orders_by_status()`: 根據狀態取得委託記錄
- `get_order_by_id()`: 根據訂單 ID 取得委託記錄
- `get_order_statistics()`: 取得委託統計資訊

**設計原則**:
- **單一職責原則 (SRP)**: 僅負責委託成交回調處理和統計分析
- **開放封閉原則 (OCP)**: 支援自訂處理函數擴展功能
