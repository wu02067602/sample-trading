...
classDiagram
    ...
    class InstrumentRetrieval {
        ...
        -api: Shioaji
        +get_contracts() list
        +search_contract(keyword) Contract
    }
    
    class QuoteSubscription {
        <<職責：訂閱即時市場報價資訊>>
        -api: Shioaji
        -subscribed_contracts: list
    ...
    class QuoteCallback {
        ...
        +on_quote_update(quote) void
    }
    
    class OrderExecution {
        <<職責：執行交易下單操作>>
        -api: Shioaji
        +place_order(contract, price, quantity) Order
...
| InstrumentRetrieval | 取得可交易的金融商品資訊，提供商品代碼、規格等基本資料 | api | get_contracts(), search_contract() |
| QuoteSubscription | 訂閱即時市場報價資訊，管理報價訂閱的啟動與停止 | api, subscribed_contracts | subscribe(), unsubscribe() |
| QuoteCallback | 接收並處理報價更新事件，將即時報價資料傳遞至相關模組 | callbacks | register_callback(), on_quote_update() |
| OrderExecution | 執行交易下單操作，處理買賣單的送出與取消 | api | place_order(), cancel_order() |
| FillReport | 取得並處理訂單成交資訊，記錄成交價格、數量與時間 | fills | get_fills(), on_fill_update() |
| OrderReport | 取得並處理委託單狀態資訊，追蹤委託單的各種狀態變化 | orders | get_orders(), on_order_update() |
