...
classDiagram
    ...
    class Config {
        ...
        -_env_file: str
        -_config: dict
        +get(key: str, default: str) Optional[str]
        +get_required(key: str) str
        +person_id: str
        +api_key: str
        +api_secret: str
        +ca_path: str
    ...
    class MarketScanner {
        ...
        +calculate_change_percent(stock: Any) Optional[float]
        --
        負責匯總和展示當前交易狀況
        提供漲跌幅、成交量、成交金額等排行查詢
    }
    ...
    class TradingSystem {
        <<交易系統主程式>>
        -_login: Optional[Login]
        -_contract_manager: Optional[ContractManager]
        -_market_scanner: Optional[MarketScanner]
        -_strategy_data_preparer: Optional[StrategyDataPreparer]
        -_quote_subscriber: Optional[QuoteSubscriber]
        -_quote_callback: Optional[QuoteCallback]
        -_strategy: Optional[MomentumStrategy]
        -_order_executor: Optional[OrderExecutor]
        -_trade_monitor: Optional[TradeMonitor]
        -_account_manager: Optional[AccountManager]
        -_api: Optional[Shioaji]
        -_is_initialized: bool
        -_subscribed_stocks: List[str]
        -_executed_trades: List[Any]
        +initialize(api_key: str, secret_key: str) None
        +get_market_ranking(count: int) List[Any]
        +start_data_preparation(interval: int, count: int) None
        +subscribe_stocks_by_change_percent(threshold: float, count: int) List[str]
        +register_signal_handler(handler: Callable) None
        +generate_daily_report() Dict[str, Any]
        +print_daily_report() None
        +shutdown() None
        -_login_to_system(config: Config) None
        -_fetch_contracts() None
        -_initialize_modules() None
        -_default_signal_handler(signal: TradingSignal) None
        -_execute_order(signal: TradingSignal) Optional[Any]
        -_monitor_order_status(trade: Any, max_wait_seconds: int, check_interval: int) bool
        -_monitor_deal_status(trade: Any, max_wait_seconds: int, check_interval: int) bool
        --
        負責控制整個交易流程的協調與管理
        整合所有模組並執行交易策略
    }
    ...
    %% 主程式協調各個模組
    TradingSystem --> Config : 讀取配置
    TradingSystem --> Login
    Config --> Login : 提供 API 金鑰
    TradingSystem --> ContractManager
    TradingSystem --> QuoteSubscriber
    TradingSystem --> QuoteCallback
    TradingSystem --> StrategyDataPreparer
    TradingSystem --> MomentumStrategy
    TradingSystem --> OrderExecutor
    TradingSystem --> TradeMonitor
    TradingSystem --> AccountManager
    TradingSystem --> MarketScanner
...
| Config | 負責讀取與管理環境變數配置，從 .env 檔案載入 API 金鑰、密鑰等配置 | ✅ 已實作 |
| TradingSystem | 負責控制整個交易流程的協調與管理，整合所有模組並執行交易策略 | ✅ 已實作 |
