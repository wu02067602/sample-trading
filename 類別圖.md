...
classDiagram
    ...
    class ShioajiClient {
        ...
        +__init__(Optional~IConfigValidator~ validator) None
        +login(LoginConfig config) Dict~str, Any~
...
- `LoginConfig`: 僅負責存儲配置資料
- `LoginConfigValidator`: 僅負責驗證配置
...
- `ShioajiClient` 依賴 `IConfigValidator` 抽象介面，而非具體實作
- 透過建構子注入，支援依賴注入模式
- 高層模組不依賴低層模組，兩者都依賴抽象
